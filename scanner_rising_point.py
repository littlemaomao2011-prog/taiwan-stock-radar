...
def check_market_risk():
    """
    大盤風控檢查：
    1. 趨勢防護：大盤跌破 20MA ➔ 紅燈熔斷保護。
    2. 波動防護：大盤短線急跌 (超過 0.8 ATR) ➔ 不停機，但將個股及格線從 60 分調高至 80 分！
    """
    print("正在分析大盤風控狀態...")
    taiex = yf.Ticker("^TWII")
    df = taiex.history(period="1mo", interval="1h")
    
    if df.empty or len(df) < 60:
        print("大盤資料不足，安全起見暫停執行。")
    ...
    crash_threshold = current_atr * 0.8

    print(f"大盤現價: {current_close:.2f} | 20MA: {current_20ma:.2f}")
    print(f"大盤當前波幅跌幅: {current_drop:.2f} | 暴跌警戒線 (0.8 ATR): {crash_threshold:.2f}")

    if current_close < current_20ma:
        return False, 60, f"🔴 紅燈熔斷：大盤跌破 20MA 生命線 ({current_close:.0f} < {current_20ma:.0f})"
    
    if current_drop > crash_threshold:
        return True, 80, "🟡 黃燈警戒：大盤短線急跌！啟動【地獄級面試】，個股及格線提高至 80 分！"
...
def analyze_stock(ticker_id):
    ...
    try:
        ...
        # ------------------------------------------
        # 2. 核心 60分K 的 KD 參數設定 (60, 3, 3) (佔 20 分)
        # ------------------------------------------
...
def main():
    ...
    if passed_stocks:
        ...
        report += f"🎯 本次選股門檻：{min_score} 分（只挑逆勢強股）\n\n"
